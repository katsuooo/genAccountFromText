...
class inputGroup:
    ...
    def getInputGroupAllList(self, input_path):
        '''input groupのリストを作成する
        textInputsフォルダのファイル名リストからグループリストを作成
        '''
        fileNames = self.fio.getFileNameFromPath(input_path)
    ...
    def readAllInputGroup(self, input_path):
        '''
        input text を読み込み、group単位でまとめたdictにして返す
        '''
        igList = self.getInputGroupAllList(input_path)
        all = AutoTree()    #def above
        #全ファイルを読み整形しjson化
        for gname in igList:
            for fname in igList[gname]:
                all[gname][fname] = self.fio.readInputText(input_path + fname)
        return all
